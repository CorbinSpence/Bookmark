...
const resolvers = {
    ...
    Mutation: {
        ...
        saveBook: async (parent, {authors, description, title, bookId, image, link}, context)=>{
            if(context.user){
                const user = User.updateOne({_id: context.user._id}, {$push: {savedBooks:{authors, description, title, bookId, image, link}}})
                return user
            }
            throw new AuthenticationError('You are not logged in.')
        },
        ...
        removeBook: async (parent, {bookId}, context)=>{
            if(context.user){
                const user = User.updateOne({_id: context.user._id}, {$pull: {savedBooks:{bookId: bookId}}})
                return user
            }
